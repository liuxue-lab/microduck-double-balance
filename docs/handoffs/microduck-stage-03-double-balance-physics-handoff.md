...
2. 保持 Actor 61 维共享契约，优先复用原篮球盲策略中恒为零的 6 维
   `body_command` 槽；冻结该槽的上球状态顺序、尺度和噪声。只有证明 6 维槽
   不足且同时完成运行时协议升级后，才允许另行提案扩维。
...
本阶段只定义双重平衡任务的 Actor/critic 观测、奖励、终止、成功判据和评估指标。先审计可部署传感器，再冻结既有 61 维 Actor 中 6 维预留槽的布局和尺度。
